...
public class DuckKingkangController {
    ...
    public SingleResponse<List<DuckKingkangVO>> byConditions() {
...
//            duckKingkangVO.setTypeName("热搜");
    ...
    public SingleResponse<Boolean> saveOrUpdate(@RequestPart("file") MultipartFile file, @RequestPart("req") DuckKingkangModel model) {
        if(file!=null){
            String filePath = iDuckKingkangService.uploadFile(file);
            model.setKingkangPic(filePath);
        }
        boolean b = iDuckKingkangService.saveOrUpdate(model);
